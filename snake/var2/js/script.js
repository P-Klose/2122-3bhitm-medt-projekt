...
class Snake {
    ...
    update() {
        ...
        if (!this.delay--) {
            ...
            if (helpers.isCollision(this.pos, food2.pos)) {
                incrementScore();
                particleSplash(food2);
                food2.spawn();
                this.total++;
            }
            if (helpers.isCollision(this.pos, food3.pos)) {
                incrementScore();
                particleSplash(food3);
                food3.spawn();
                this.total++;
            }
            ...
            if (helpers.isCollision(this.pos, food5.pos)) {
                incrementScore();
                particleSplash(food5);
                food5.spawn();
                this.total++;
            }
            if (helpers.isCollision(this.pos, food6.pos)) {
                incrementScore();
...
function particleSplash(foodItem) {
    for (let i = 0; i < splashingParticleCount; i++) {
        let vel = new helpers.Vec(Math.random() * 6 - 3, Math.random() * 6 - 3);
        let position = new helpers.Vec(foodItem.pos.x, foodItem.pos.y);
        particles.push(new Particle(position, currentHue, foodItem.size, vel));
    }
}
